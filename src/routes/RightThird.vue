...
<template>
  <div v-if="hasOpenFiles" class="overflow-hidden bg-neutral-800 h-full flex-1 flex flex-col justify-between">
      <Tabs />
    <div
    ...
    >
      <editor
          ...
          @init="editorInit"
      ></editor>
      <div class="h-8 w-full bg-blue-400 flex flex-wrap items-center px-4 gap-2">
          <div id="statusBar"></div>
          <select v-model="selectedTheme" @change="onChangeTheme" class="rounded-md border-0 dark:text-stone-50 dark:bg-slate-700 m-0 px-2 py-0">
              <option v-for="theme in themeList" :value="theme" :key="theme">{{  theme.name }}</option>
          </select>
          <select v-model="selectedLanguage" @change="onChangeLanguage" class="rounded-md border-0 dark:text-stone-50 dark:bg-slate-700 m-0 px-2 py-0">
              <option v-for="language in modeList" :value="language" :key="language">{{  language.name }}</option>
          </select>
          <div class="font-bold">{{ $store.getters?.tab?.data?.includes("\r\n") ? 'CRLF' : 'LF'}}</div>
      </div>
