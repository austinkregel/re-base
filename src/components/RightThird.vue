...
::-webkit-scrollbar {
  width: 8px;
  height: 8px;
  @apply scrollbar-thumb-neutral-600 scrollbar-track-neutral-700;
}
...
<template>
  <div v-if="hasOpenFiles && $store.state.open.tab" class="overflow-hidden bg-neutral-800 flex-grow flex-1 flex flex-col justify-between">
      <Tabs />
    <div
    ...
    >
      <editor
          ref="editor"
          class="relative w-full h-full min-h-64 flex-grow"
          v-model:value="$store.getters.tab.data"
          :options="$store.getters.settings.editor"
      ...
      <div class="h-6 w-full bg-blue-400 flex flex-wrap justify-end items-center gap-2 px-4">
          <div id="statusBar"></div>
          <select v-model="selectedTheme" @change="onChangeTheme" class="rounded-md border-0 dark:text-stone-50 dark:bg-slate-700 m-0 px-2 py-0 text-xs">
              <option v-for="theme in themeList" :value="theme" :key="theme">{{  theme.name }}</option>
          </select>
          <select v-model="selectedLanguage" @change="onChangeLanguage" class="rounded-md border-0 dark:text-stone-50 dark:bg-slate-700 m-0 px-2 py-0 text-xs">
              <option v-for="language in modeList" :value="language" :key="language">{{  language.name }}</option>
          </select>
          <div class="font-bold text-xs">{{ $store.getters?.tab?.data?.includes("\r\n") ? 'CRLF' : 'LF'}}</div>
      </div>
...
const editorTheme = getLocalStorage('ace-editor-theme', {});
if (editorTheme && editorTheme?.name) {
  // import('ace-builds/src-noconflict/theme-' + editorTheme.name);
}

const Editor = VAceEditor;
const listOfModes = ace.require("ace/ext/modelist")

const listOfThemes = ace.require("ace/ext/themelist")

const { StatusBar } = ace.require('ace/ext/statusbar');
...
export default {
  ...
  data() {
    return {
      languages: [],
      modeList: listOfModes.modes,
      themeList: Object.values(listOfThemes.themes).filter(t => t.isDark),
  ...
  computed: {
    hasOpenFiles() {
      if (!this.$store.getters.tab || !this.$store.getters?.tab?.data) {
          return false;
      }
  ...
  watch: {
    selectedTheme(newValue, oldValue) {
      setLocalStorage('ace-editor-theme', newValue);
    },
    selectedLanguage(newValue, oldValue) {
      setLocalStorage('ace-editor-language', newValue);
    },

    '$store.getters.tab'(newValue, oldV) {
        console.log('new tab name', newValue.name);
      if (!newValue) {
        return;
      }

      this.selectedLanguage = listOfModes.getModeForPath(this.$store.getters.tab.name);
    },
  },
  methods: {
      onChangeTheme(theme){
        console.log(this.selectedTheme);
        // ace.require(this.selectedTheme.theme)
      },
      onChangeLanguage(lang){
          // ace.require(this.selectedTheme.mode)
          console.log(this.selectedLanguage);
      },
    editorChange(obj) {
        if (!storeLeakForAceEditor) {
        return;
      }
      ...
      if (Object.values(storeLeakForAceEditor.getters.openFiles).filter(file => file?.isDirty ?? false).length > 0)  {
    ...
    editorInit(editor, store) {
      ...
      createTerminal() {
        if (this.$store.getters.terminalsOpen) {
            this.$store.dispatch('destroyTerminal', this.$store.getters.terminal);
            this.$store.commit('toggleTerminal');
            return;
        }
